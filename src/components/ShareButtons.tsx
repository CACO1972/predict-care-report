...
const ShareButtons = ({ patientName, successProbability, pronosticoLabel }: ShareButtonsProps) => {
  const getShareText = () => {
    const name = patientName || 'Yo';
    return `¡${name} acabo de evaluar mis posibilidades de implantes dentales con ImplantX! 🦷✨ Mi probabilidad de éxito es del ${successProbability}%. Evalúa las tuyas gratis:`;
  };
  ...
  const handleInstagramShare = () => {
    ...
    toast.success("¡Texto copiado! Pégalo en tu historia de Instagram", {
      description: "El texto está listo para compartir"
    });
  };
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: 'ImplantX - Evaluación de Implantes Dentales',
          text: getShareText(),
          url: shareUrl,
        });
      ...
      <div className="flex flex-col sm:flex-row items-center gap-3">
        <div className="flex-1 text-center sm:text-left">
          ...
          <p className="text-xs text-muted-foreground">
            Ayuda a otros a conocer su salud dental
          </p>
        </div>
